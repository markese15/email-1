...
public class NotificationController {
    ...
    public ResponseEntity sendNotification(@RequestBody NotificationDTO payload){
        try{
            ...
            System.out.println("student  " + studentToNotify);
            if (studentToNotify == null)
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Non sono riuscito a trovare lo studente ");
            emailService.sendTo(studentToNotify.getEmail(), payload.getTitle(), payload.getText());
            return ResponseEntity.status(HttpStatus.OK).build();
        }catch(Exception e){
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("server non risponde");
        }
    }
}
